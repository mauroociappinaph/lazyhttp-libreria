 ...
 * utilizando los resource accessors y combinando datos de distintos endpoints
 */
...
/**
 * Ejemplo de uso del resource accessor para obtener grupos
 * El resource accessor permite usar la sintaxis: http.get['endpoint'](...)
 */
async function obtenerGrupos() {
  console.log("\n👥 Obteniendo grupos utilizando resource accessor...");

  try {
    // Usar el resource accessor
    const response = await http.get["group_list"]({
...
/**
 * Ejemplo de uso del resource accessor para obtener licencias
 */
async function obtenerLicencias() {
  console.log("\n📜 Obteniendo licencias utilizando resource accessor...");

  try {
    const response = await http.get["license_list"]();
...
/**
 * Ejemplo avanzado: Búsqueda por facetas con resource accessor
 */
async function busquedaAvanzada(query, filtros = {}) {
  ...
  try {
    ...
    // Usar el resource accessor para la búsqueda
    const response = await http.get["package_search"]({
      params,
    });
...
/**
 * Ejemplo avanzado: Guardar un informe de análisis
 */
...
async function main() {
  try {
    ...
    // 1. Obtener grupos usando resource accessor
    const grupos = await obtenerGrupos();

    // 2. Obtener licencias usando resource accessor
