...
import { AST, ASTNode, DuplicateDetectionError, ErrorType } from '../../../duplicate-detector/types/index';
...
describe('TreeSimilarityCalculator', () => {
  ...
  describe('constructor and configuration', () => {
    ...
    it('should throw error for invalid threshold', () => {
      expect(() => calculator.setThreshold(-0.1)).toThrow(DuplicateDetectionError);
      expect(() => calculator.setThreshold(1.1)).toThrow(DuplicateDetectionError);
    });
  });
  ...
  describe('calculateSimilarity', () => {
    ...
    beforeEach(() => {
      ...
      simpleAST1 = createSimpleAST('function add(a, b) { return a + b; }', 'test1.ts');
      simpleAST2 = createSimpleAST('function add(x, y) { return x + y; }', 'test2.ts');
      identicalAST = createSimpleAST('function add(a, b) { return a + b; }', 'test3.ts');
      differentAST = createComplexAST('class Calculator { multiply(a, b) { return a * b; } }', 'test4.ts');
    });

    it('should calculate similarity between identical ASTs', () => {
      const similarity = calculator.calculateSimilarity(simpleAST1, identicalAST);
    ...
    it('should calculate similarity between similar ASTs', () => {
      ...
      expect(['syntactic', 'semantic', 'structural']).toContain(similarity.type);
    });

    it('should calculate low similarity between different ASTs', () => {
      const similarity = calculator.calculateSimilarity(simpleAST1, differentAST);
    ...
    it('should return consistent results for same input', () => {
      const similarity1 = calculator.calculateSimilarity(simpleAST1, simpleAST2);
      const similarity2 = calculator.calculateSimilarity(simpleAST1, simpleAST2);
    ...
    it('should be symmetric', () => {
      const similarity1 = calculator.calculateSimilarity(simpleAST1, simpleAST2);
      const similarity2 = calculator.calculateSimilarity(simpleAST2, simpleAST1);

      expect(Math.abs(similarity1.score - similarity2.score)).toBeLessThan(0.01);
    });
  });
  ...
  describe('similarity types', () => {
    it('should identify syntactic similarity', () => {
      const ast1 = createSimpleAST('function test() { return true; }', 'test1.ts');
      const ast2 = createSimpleAST('function test() { return true; }', 'test2.ts');
    ...
    it('should identify semantic similarity', () => {
      ...
      const ast2 = createComplexAST('class Math { add(x, y) { return x + y; } }', 'sem2.ts');
  ...
  describe('error handling', () => {
    it('should handle malformed ASTs gracefully', () => {
      ...
      expect(() => calculator.calculateSimilarity(malformedAST, validAST)).toThrow(DuplicateDetectionError);
    });
    ...
    it('should provide meaningful error messages', () => {
      ...
      try {
        ...
        expect((error as DuplicateDetectionError).type).toBe(ErrorType.ANALYSIS_TIMEOUT);
      }
    });
  });
});
...
function getTokenType(token: string): string {
  ...
  if (['function', 'class', 'const', 'let', 'var', 'return', 'if', 'else'].includes(token)) return 'Keyword';
